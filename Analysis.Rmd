...
Data set showing the weight of an mammal at NEON sites:
```{r subset-out-weight-data}
# just look at the weight of the mammal
weight_data <- subset(mam_pertrapnight,
                        functionalGroup == "weight")
```

Box plot of the weight of small mammals at NEON sites:
```{r boxplot-of-functional-types}
# weight of small mammals at diffrent sites in the NEON data
boxplot(weight ~ siteID,
        data = mam_pertrapnight,
       main = "Weight of Small Mammals at NEON sites",
        xlab = "site ID",
        ylab = "weight (grams)",
       las = 2)
```
